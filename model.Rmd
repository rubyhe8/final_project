...
To build off of our regression analysis, we were curious to see what is the secret recipe to highly rated films. We decided to first fit a `lasso` model using `glmnet` to give us an idea of what variables impact average rating the most.
...
We can see that using `lasso`, the following predictors were chosen:  
...
We would like to include total number of watches as the frequency with which a movie is viewed will have an effect on the number of ratings it receives and consequently its average rating. We decided to exclude lowest and highest (number of lowest ratings and number of highest ratings) as this may be repetitive with the average rating variable. 
