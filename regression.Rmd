...
The coefficient for `runtime` is **0.00258**, which represents the estimated increase in average rating for each unit increase in runtime mintues. 

The model's p-value is **1.04e-59**, which indicates that runtime is a significant predictor of average rating.

Though, based on the numbers a linear model makes sense, when looking at the graphs there does not seem to be an apparent linear realtionship and therefore further analysis was done in order to see if there is a better model for the association between runtime and average rating.  
...
```
When Runtime is into the full model including watches, list_appearances and fans, the coefficient became way smaller to 1.57e-0.3 and the p-value decreased to 7.89e-27. Which makes it more signifanct yet when put with all the other predictors. 
```{r, message = FALSE}
...
When the runtime variable  model was put into a log transformation there is improvement in the model in terms of visuals but the and the p-value is *9.37e-09* is bigger than the one in the linear model but still stands significant. 
...
```
Dichotomizing the variables to greater and less than 90 mins made the model more accurate and better to visualize. Runtime seems to be a hard variable to work with in general given the amount of possible outcomes and different run times. Though when put in the overall model it works well as a predictor and the longer the moive the higher the rating for movies over 90 mins but the longer the movie the lower the rating for movies under 90 mins. This is a interesting comparision to big more into. 
...
While the adjusted runtime model has the highest R-squared, only 19.1% of average rating variance can be explained. More characteristics seem to influence a movie's average rating.
